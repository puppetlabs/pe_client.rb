...
RSpec.describe PEClient::Resource::PuppetCAV1::CertificateStatus do
  ...
  describe "#get" do
    context "when retrieving a signed certificate" do
      it "gets the certificate status for the specified node" do
        response_body = {
          "name" => certname,
          "state" => "signed",
    ...
    context "when retrieving a requested certificate" do
      it "gets the certificate status for a pending request" do
        response_body = {
          "name" => certname,
          "state" => "requested",
          "fingerprint" => "A1:B2:C3:D4:E5:F6:A7:B8:C9:D0:E1:F2:A3:B4:C5:D6:E7:F8:A9:B0:C1:D2:E3:F4:A5:B6:C7:D8:E9:F0:A1:B2",
          "fingerprints" => {
            "SHA1" => "A1:B2:C3:D4:E5:F6:A7:B8:C9:D0:E1:F2:A3:B4:C5:D6:E7:F8:A9:B0",
            "SHA256" => "A1:B2:C3:D4:E5:F6:A7:B8:C9:D0:E1:F2:A3:B4:C5:D6:E7:F8:A9:B0:C1:D2:E3:F4:A5:B6:C7:D8:E9:F0:A1:B2",
            "SHA512" => "A1:B2:C3:D4:E5:F6:A7:B8:C9:D0:E1:F2:A3:B4:C5:D6:E7:F8:A9:B0:C1:D2:E3:F4:A5:B6:C7:D8:E9:F0:A1:B2:C3:D4:E5:F6:A7:B8:C9:D0:E1:F2:A3:B4:C5:D6:E7:F8:A9:B0:C1:D2:E3:F4:A5:B6:C7:D8:E9:F0:A1:B2"
          },
  ...
  describe "#list" do
    context "when listing all certificates without filter" do
      it "gets information about all known certificates" do
        response_body = [
          {
    ...
    context "when filtering by requested state" do
      it "gets only requested certificates" do
        response_body = [
          {
            "name" => "node2.example.com",
            "state" => "requested",
    ...
    context "when filtering by signed state" do
      it "gets only signed certificates" do
        response_body = [
          {
            "name" => "node1.example.com",
            "state" => "signed",
        ...
        result = resource.list(state: "signed")
        expect(result).to be_an(Array)
        expect(result.all? { |cert| cert["state"] == "signed" }).to be true
      end
    end

    context "when filtering by revoked state" do
      it "gets only revoked certificates" do
        response_body = [
          {
            "name" => "node3.example.com",
            "state" => "revoked",
        ...
        result = resource.list(state: "revoked")
        expect(result).to be_an(Array)
        expect(result.all? { |cert| cert["state"] == "revoked" }).to be true
      end
    end
  end

  describe "#update" do
    context "when signing a certificate request" do
      it "updates the certificate status to signed" do
        response_body = {
          "name" => certname,
          "state" => "signed",
          "fingerprint" => "C2:FD:F0:1A:9F:A7:BD:F2:87:D6:0D:F8:D0:43:B1:E0:63:E2:5F:33:92:A2:B9:93:8A:56:20:38:E6:70:A4:16",
        ...
        result = resource.update(certname, "signed")
        expect(result).to be_a(Hash)
        expect(result["name"]).to eq(certname)
        expect(result["state"]).to eq("signed")
      end
    ...
    context "when signing a certificate request with a custom TTL" do
      it "updates the certificate status to signed with specified TTL" do
        response_body = {
          "name" => certname,
          "state" => "signed",
          "fingerprint" => "C2:FD:F0:1A:9F:A7:BD:F2:87:D6:0D:F8:D0:43:B1:E0:63:E2:5F:33:92:A2:B9:93:8A:56:20:38:E6:70:A4:16",
    ...
    context "when signing with numeric TTL in seconds" do
      it "updates the certificate status to signed with numeric TTL" do
        response_body = {
          "name" => certname,
          "state" => "signed",
          "fingerprint" => "C2:FD:F0:1A:9F:A7:BD:F2:87:D6:0D:F8:D0:43:B1:E0:63:E2:5F:33:92:A2:B9:93:8A:56:20:38:E6:70:A4:16"
        ...
        stub_request(:put, "#{base_url}/puppet-ca/v1/certificate_status/#{certname}")
          .with(body: {desired_state: "signed", cert_ttl: 31536000})
          .to_return(
            status: 200,
            body: response_body.to_json,
            headers: {"Content-Type" => "application/json"}
        ...
        result = resource.update(certname, "signed", cert_ttl: 31536000)
        expect(result).to be_a(Hash)
        expect(result["state"]).to eq("signed")
      end
    end

    context "when revoking a certificate" do
  ...
  describe "#delete" do
    context "when deleting SSL information for a host" do
    ...
    context "when deleting a non-existent host" do
      it "handles deletion of a host without SSL information" do
        hostname = "nonexistent.example.com"
        response_body = "Nothing was deleted"
        ...
        result = resource.delete(hostname)
        expect(result).to be_a(String)
        expect(result).to eq("Nothing was deleted")
      end
    end
  end
end
