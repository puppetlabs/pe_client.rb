...
RSpec.describe PEClient::Resource::OrchestratorV1::Plans do
  ...
  describe "#get" do
    ...
    it "gets information about a specific plan with environment parameter" do
      stub_request(:get, "#{base_url}/orchestrator/v1/plans/mymodule/myplan?environment=production")
        .with(headers: {"X-Authentication" => api_key})
        .to_return(
          status: 200,
          body: '{"name":"mymodule::myplan","environment":"production","metadata":{"description":"My plan"}}',
          headers: {"Content-Type" => "application/json"}
        )

      response = resource.get(module_name: "mymodule", plan_name: "myplan", environment: "production")
      expect(response).to eq({"name" => "mymodule::myplan", "environment" => "production", "metadata" => {"description" => "My plan"}})
    end
  end
end
